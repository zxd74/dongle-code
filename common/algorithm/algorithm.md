...
## 设计思想
| 设计思想       | 核心思想                                                                 | 是否保证最优解 | 时间复杂度          | 空间复杂度       | 关键对比点                                                                 | 经典应用案例                     |
|----------------|--------------------------------------------------------------------------|----------------|---------------------|------------------|----------------------------------------------------------------------------|----------------------------------|
| **分治**       | 1. 分解问题<br>2. 递归解决子问题<br>3. 合并子问题的解                     | ✔️             | 通常 \(O(n \log n)\) | \(O(\log n)\)    | - 子问题独立<br>- 必须合并解<br>- 无重复计算                               | 归并排序、快速排序、最近点对问题 |
| **回溯**       | 1. 试探性搜索<br>2. 递归+剪枝<br>3. 失败则回退（撤销选择）                | ✔️（所有解）    | 指数级 \(O(2^n)\)    | \(O(n)\)         | - 穷举+剪枝<br>- 需存储路径<br>- 适合组合/排列问题                         | N皇后、数独、全排列              |
| **动态规划**   | 1. 最优子结构<br>2. 重叠子问题<br>3. 存储中间状态（DP表/备忘录）           | ✔️             | 多项式 \(O(n^2)\)    | \(O(n)\)或\(O(n^2)\) | - 空间换时间<br>- 避免重复计算<br>- 自底向上或自顶向下                     | 0-1背包、LCS、斐波那契数列       |
| **贪心**       | 1. 局部最优选择<br>2. 不可回退                                            | ❌（可能非最优）| 通常 \(O(n \log n)\) | \(O(1)\)         | - 高效但不保证全局最优<br>- 需证明贪心选择性                               | Dijkstra、霍夫曼编码、任务调度   |
| **分支限界**   | 1. 状态空间树搜索<br>2. 优先级队列管理节点<br>3. 剪枝非优分支              | ✔️             | 指数级（优于回溯）    | \(O(b^d)\)       | - 通过界限函数剪枝<br>- 适合组合优化问题<br>- 需维护优先级队列              | 旅行商问题（TSP）、任务分配      |

**关键对比总结**
1. **最优解保证**：动态规划、分治、分支限界 > 回溯（所有解） > 贪心（可能非最优）  
2. **效率**：贪心 ≈ 分治 > 动态规划 > 分支限界 > 回溯  
3. **空间开销**：动态规划 > 分支限界 ≈ 回溯 > 分治 > 贪心  
4. **适用场景**：  
   - 子问题独立 → **分治**  
   - 重叠子问题 → **动态规划**  
   - 穷举+剪枝 → **回溯**/**分支限界**  
