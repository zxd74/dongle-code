...
## Hash分类
根据任务中的某些关键信息进行Hash运算，将相同Hash请求分配到相同服务上，常见有源地址Hash、目标地址Hash、Session id hash、用户ID Hash等等
...
### 设备状态
1. down 当前单个server暂不可用
2. weight 当前server权重
3. max_fails 最大失败数，超出返回proxy_next_upstream 模块
4. fail_timeout max_fails失败后暂停时间
5. backup 其他非backup机器down或压力大时，请求该服务器，故该服务压力最小
###  多组负载
1. client_body_in_file_only 设置on，可记录client post数据记录到文件用于debug
2. client_body_temp_path    设置记录文件目录，最多三层目录
3. location 对URL匹配，可以重定向或新代理，负载均衡